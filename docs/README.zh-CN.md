...
Claude 每次提交时清扫一遍：按项目自己的规则，格式整齐、没有 lint 问题、类型正确。
...
- **清扫**：Claude 执行 `git commit` 时，broom 格式化要提交的文件，检查改动的行，对项目做类型检查，问题解决前不让提交。
- **看见**：为 Go、TS/JS、Rust、Python 提供语言服务器，Claude 边改边看到诊断。
...
大多数时候不用管：Claude 提交时 broom 自己会跑。想先看看的话，用自己的话让它清扫：
...
| `format_scope` | `changed` | `function`（整个改动过的函数）、`file` |
